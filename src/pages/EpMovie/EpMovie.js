...
const EpMovie = () => {
  ...
  useEffect(() => {
    ...
    const fetchEp = async () => {
      const response = await axios.get(
        `http://localhost:1412/api/admin/episode/getBymovie/all/${movieId}`
      );
    ...
    const fetchcomment = async (page) => {
      const response = await axios.get(
        `http://localhost:1412/api/user/comment/episode/byep/${id}?page=${page}&limit=10`
      );
